...
<body>
    ...
    <?php  
        if(!empty($_GET['page'])) {
            $page = $_GET['page'];
            if(!empty($pages[$page])){ //to check whether $page is in $pages via in_array
                echo file_get_contents("pages/{$page}.html");  //wouldn't execute php code because of `file_POST_contents`
            }
        }
    ?>
</body>
